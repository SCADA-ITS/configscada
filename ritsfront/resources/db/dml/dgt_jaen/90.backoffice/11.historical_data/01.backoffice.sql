...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'historical_data') THEN
	...
	VALUES(11, 'historical_data_backoffice', NULL, NULL, 'jdbc:postgresql://172.21.3.203:5430/rits', 'rits', 'uMuD7Uy0gpCRL4Jo1w6SOQ==', 'historical_data',
...
LEFT JOIN LATERAL (
    SELECT cit.description, hir.incident_report_id
    FROM hist.ims_incident_reports hir
    JOIN conf.ims_incident_types cit 
      ON cit.incident_type_id = hir.incident_type_id
    WHERE hir.incident_report_id = CAST(regexp_replace(ac.comment, '^ImsIncidentReport:', '', 'i') AS INTEGER)
    LIMIT 1
) AS it ON TRUE
