...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'tbl_nogales_gip') THEN
		...
		create table nogales_gip.participant_types (
			id serial not null,
			alias varchar null,
		...
		alter table nogales_gip.participants add constraint fk_participant_type_id foreign key (participant_type_id) references nogales_gip.participant_types(id) ON DELETE CASCADE;
