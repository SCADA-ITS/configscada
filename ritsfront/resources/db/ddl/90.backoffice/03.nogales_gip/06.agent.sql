...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'tbl_nogales_gip') THEN
		...
		create table nogales_gip.agents (
			id serial not null,
			alias varchar null,
		...
		alter table nogales_gip.interrupt_agents add constraint fk_interrupt_agents_incident_id foreign key (incident_id) references nogales_gip.incidents(id) ON DELETE CASCADE;
		alter table nogales_gip.interrupt_agents add constraint fk_agents_id foreign key (agents_id) references nogales_gip.agents(id) ON DELETE CASCADE;
