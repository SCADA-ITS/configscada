...
const PARAM_COMMAND_VALUE_ID = "ElementTypeParam:87:2:4";

const ELEMENT_TYPE_STATES_NO_TREATMENT = ["ElementTypeState:87:0", "ElementTypeState:87:2"];
const ELEMENT_TYPE_STATES_UP = "ElementTypeState:87:1";
const ELEMENT_TYPE_STATES_ALARM = "ElementTypeState:87:3";
	...
	render(elementInfo) {
		...
		if (this.gState) {
			...
			if (cmdGroupValue && treatmentValue) {
				switch (true) {
					case cmdGroupValue == 'true' && elementInfo.state.id == ELEMENT_TYPE_STATES_UP:
					...
					case cmdGroupValue == 'false' && elementInfo.state.id == ELEMENT_TYPE_STATES_UP:
						this.gLevel25.attr({ "fill-opacity": 1 });
						this.gLevel50.attr({ "fill-opacity": 1 });
					...
					case elementInfo.state.id == ELEMENT_TYPE_STATES_ALARM:
						this.gLevel25.attr({ "fill-opacity": 1 });
