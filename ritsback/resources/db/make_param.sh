...
backupProc() {
	...
	psql postgresql://rits:rits@$psqlDB/rits -c "SET client_min_messages TO WARNING; DROP SCHEMA IF EXISTS $backup_schema CASCADE;" >/dev/null
	
	echo "Creating schema $backup_schema..."
	psql postgresql://rits:rits@$psqlDB/rits -c "SET client_min_messages TO WARNING; CREATE SCHEMA $backup_schema;" >/dev/null
	...
	do
		...
		#echo -e "\t$sql"
		psql postgresql://rits:rits@$psqlDB/rits -c "$sql" >/dev/null
...
mainProc() {
	...
	case $project in
		maqueta_dev )		
			psql postgresql://rits:rits@$psqlDB/rits -c "UPDATE conf.io_controller_module_values SET value='http://172.17.0.1:8080/rest/v1' WHERE module_type_id=1 AND module_id=1 AND module_type_param_id=13;"
			psql postgresql://rits:rits@$psqlDB/rits -c "UPDATE conf.io_controller_module_values SET value='http://172.17.0.1:8080/rest/v2/login' WHERE module_type_id=1 AND module_id=1 AND module_type_param_id=14;"
			psql postgresql://rits:rits@$psqlDB/rits -c "DROP SCHEMA hist CASCADE;"
			psql postgresql://rits:rits@$psqlDB/rits -c "CREATE SCHEMA hist;";;
	esac
	...
	then
		psql postgresql://rits:rits@$psqlDB/rits -c "INSERT INTO conf.system_param_values (system_param_id, value, enabled, visible, created_at, updated_at) VALUES (10, '$commit', true, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);"
		psql postgresql://rits:rits@$psqlDB/rits -c "INSERT INTO conf.system_param_values (system_param_id, value, enabled, visible, created_at, updated_at) VALUES (11, '$author', true, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);"
		psql postgresql://rits:rits@$psqlDB/rits -c "INSERT INTO conf.system_param_values (system_param_id, value, enabled, visible, created_at, updated_at) VALUES (12, '$date', true, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);"
	fi
}
