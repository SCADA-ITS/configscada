...
backupProc() {
	...
	psql postgresql://dbo_controltrafico:ha9t5h57Emh4h1@$psqlDB/controltrafico -c "SET client_min_messages TO WARNING; DROP SCHEMA IF EXISTS $backup_schema CASCADE;" >/dev/null
	
	echo "Creating schema $backup_schema..."
	psql postgresql://dbo_controltrafico:ha9t5h57Emh4h1@$psqlDB/controltrafico -c "SET client_min_messages TO WARNING; CREATE SCHEMA $backup_schema;" >/dev/null
...
mainProc() {
	...
	cd ..
	cd dml/master
	/bin/bash ./dml_param_navarra_lab.sh $psqlHost $psqlPort
	cd ..
	cd $project
