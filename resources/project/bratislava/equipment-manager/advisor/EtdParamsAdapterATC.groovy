...
class EtdParamsAdapterATC {
	...
	//boolean etd_info(Element element, String value, List<ElementValue> elementValues) {
		...
		List<ElementValue> listElements;
				log.debug("Ejecutando EtdParamsAdapterATC Element" + element.getElementTypeId() + ":" + element.getId());
		...
		try {
			...
			if (elementValues != null) {
				...
				for (ElementHierarchy child : childs){
					...
					log.debug("TIMESTAMP->  " + String.valueOf(etd_info.get(detArrayPos).timestamp))
