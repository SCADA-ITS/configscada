...
def void signalling2LCS(SignallingCommand signallingCommand, String dataSourceXid, MangoDriver driver, Element element, org.apache.logging.log4j.Logger log) {	
	final int NUM_ZONES_2LCS = 4;
	...
	if (priorityNew  == 0 && !incidentType.equals(" ")){
		...
		for(int j = 0; j < NUM_ZONES_2LCS; j++){
			graphics.add("");
			texts.add("");
		}
		...
		for (int i = 0; i < object.size(); i++){
			...
			if(!CollectionUtils.isEmpty(object[i].getGraphics())){
				...
				for(int j = 0; j < object[i].getTexts().size() ; j++){
					textValue = "[tr" + (70-((object[i].getTexts()[j].getValue().size()-1)*3)) + ",49,0,0]";
			...
			if(graphics.get(zone)!="")
				multi += "[g" + graphics.get(zone) + "," + (((int)(zone/2)*48)+1) + "," + (((zone%2)*48)+1) + "]";
