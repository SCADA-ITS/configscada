...
def void signalling3LCS(SignallingCommand signallingCommand, String dataSourceXid, MangoDriver driver, Element element, org.apache.logging.log4j.Logger log) {	
	...
	if (priorityNew  == 0 && !incidentType.equals(" ")){
		...
		Object object = objectMapper.readValue(signallingCommand.signallingParams.get(PARAM_MESSAGE_ID).getValue(), Zone[].class);	
		...
		for(int j = 0; j < NUM_ZONES_3LCS; j++){
			graphics.add("");
			texts.add("");
		}
		...
		for (int i = 0; i < object.size(); i++){
			...
			}else if(!CollectionUtils.isEmpty(object[i].getTexts())) {
