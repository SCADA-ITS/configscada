...
class InputAdapter_VMS {
	...
	final String TAG_TEXTO = "[fo1][tr1,1,0,0]";
	...
	boolean content(Element element, String value, List<ElementValue> elementValues) {
		...
		String result = construirVMS(comprobarHex(value), element);
	...
	public String construirVMS(String content, Element element) {
		...
		texts.add(sustituirCaracter(content,cad_character,cad_ascii).replace(TAG_TEXTO, "").replace("[nl]","#"));
			...
			for(int n_zone = 1; n_zone <= num_zones; n_zone++){
				...
				if(zone_type && zone_type != null){
					if(zone_type.equals(DUAL)){
						...
						if(zone.fillGraphic(graphics, element, n_zone) == 0){
	...
	class Zone{
		...
		public int fillGraphic(List<String> message, Element element, int numZone) {
			
			Integer values = 0;
			Integer alternances = 0;

			values = obtenerGraphic(element,message.get(0),numZone);
		...
		public int obtenerGraphic(Element element, String pictoValue, Integer numZone){
			...
			for(VmsGraphicGraphicGroupValue groupValue : vmsGraphicGraphicGroupValue){
				if(pictoValue.equals(groupValue.getValue())){
