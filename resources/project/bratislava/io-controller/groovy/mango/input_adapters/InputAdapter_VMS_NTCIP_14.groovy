...
class InputAdapter_VMS 
{
	...
	public String construirVMS(String content, Element element)
	{
		...
		int alternance, flashing_on, flashing_off, numzone, aux_graphic, num_lineas;
		...
		if (CONTENT_COPY.contains("g"))
			aux_graphic = 1;			
		if (CONTENT_COPY.contains("tr"))
		{
			while(CONTENT_COPY)
			{
		...
		if(num_zones && num_zones != null)
		{
			for(int n_zone = 1; n_zone <= num_zones; n_zone++)
			{
				...
				if(zone_type && zone_type != null)
				{
					...
					{
						if(aux_graphic != 0)
							zone.fillGraphic(aux_graphic, element, n_zone);
	...
	class Zone
	{
		...
		public void fillGraphic(int aux_graphic, Element element, int numZone)
		{
			...
			values = obtenerGraphic(element,aux_graphic,numZone);
