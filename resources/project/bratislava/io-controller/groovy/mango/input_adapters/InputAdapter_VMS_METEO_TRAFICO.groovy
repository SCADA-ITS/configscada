...
class InputAdapter_VMS {
	...
	static final Integer GRAPHIC_TEMP_882 = 7;
	...
	boolean content(Element element, String value, List<ElementValue> elementValues) {
		...
		if (result == null || result.equals("")){
			...
			elementValue.setParamTypeId(2L);
			elementValue.setElementId(element.getId());
			//elementValue.setValue('[{\"zone\":1,\"alternance_ms\":5000,\"flashing_on_ms\":0,\"flashing_off_ms\":0,\"graphics\":[{\"id\":1, \"value\":882, \"alternance\":880}]}, {\"zone\":2, \"alternance_ms\":5000,\"flashing_on_ms\":0,\"flashing_off_ms\":0,\"graphics\":[], \"texts\":[{"id":1,"value":"37","alternance":"","color":"#FFFFFF"}, {"id":2, "value":"", "alternance":"D4", "color":"#FFFFFF"}],"align":"center"}]'
...
//);
	...
	public String construirVMS(String content, Element element) {
		...
		int alternance, flashing_on, flashing_off, num_lineas, zona_text, align;
		...
		for (String message : messages) {
			...
			for(int i = 0; i < messages.size(); i++){
				if (messages[i].charAt(0) == "g"){
					...
					}else if(x_zone == 42 && y_zone == 16){
						texts.set(4,messages[i+1])
					}
				}
			}
			...
			for(int n_zone = 1; n_zone < num_zones + 1; n_zone++){
				...
				if(zone_type && zone_type != null){
					...
					if(zone_type.equals(GRAPHIC)){
						...
						if(n_zone <= 5){
							zone.fillText(texts, num_lineas, n_zone-2);
						}else if(n_zone > 5){
							zone.fillText(texts, num_lineas, n_zone-2);
						}
						else{
	...
	class Zone{
				...
				for(int contador = 1; contador <= numLineas; contador++) {
					...
					if (n_zone <= 1 && aux_value.charAt(0) == "-" ){
						if (!messages[4]){
							...
							textLine = new Text(contador, aux_value, "", "#FF0000");
						}
						
						if(messages[4] && n_zone == 0){

							textLine_traffic = new Text(2, "", messages[4], "#FFFFFF");
							texts.add(textLine_traffic);
						}
					}else{
						if (!messages[4]){
							...
							textLine = new Text(contador, aux_value, "", "#FFFFFF");
						}
						if(messages[4] && n_zone == 0){
		...
		public void fillGraphic(List<String> message, Element element, int numZone) {
			...
			Integer alternances = 0;
				if (numZone == 1 && message.size() == 3){
					alternances = obtenerGraphic(element,Integer.parseInt(message.get(numZone + 1)),numZone);
				}
				if (message[0] == '1' && message[1] == '2') {
					values = obtenerGraphic(element,GRAPHIC_TEMP_882,numZone);
				}
