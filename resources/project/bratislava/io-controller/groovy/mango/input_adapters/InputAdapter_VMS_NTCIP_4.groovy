...
class InputAdapter_VMS {
	...
	static final Integer POS_TEXT_1 = 1;
	static final Integer POS_TEXT_2 = 0;
	static final Integer NUM_TEXT_ZONES = 2;
	static final Integer ZONA_1 = 1;
	static final Integer ZONA_2 = 2;
	static final Integer ZONA_3 = 3;
	...
	public String construirVMS(String content, Element element) {
		...
		content = sustituirCaracter(content,cad_character,cad_ascii);
		content = content.replace("[nl]", "#");
		
		int graphic_index = content.indexOf("[g");
		int text_big_index = content.indexOf("[tr40");
		int text_small_index = content.indexOf(",45,0,0]");
		
		for(int i = 0; i<NUM_TEXT_ZONES; i++){
			texts.add("");
		}
		
		//Primero busco el grafico si existe
		if (graphic_index != -1)
			graphics.add(content.substring(graphic_index + 2, content.indexOf(",", graphic_index + 2)));
		
		//A continuación busco el texto pequeño si existe
		if(text_small_index != -1){
			content_aux = content.substring(text_small_index + ",45,0,0]".length());
			texts.set(POS_TEXT_2, content_aux);
		}
		
		//Finalmente busco el texto grande si existe		
		if(text_big_index != -1){
			content_aux = content.substring(text_big_index);
			content_aux = content_aux.substring(content_aux.indexOf("]") + 1);
			
			if(content_aux.indexOf("[") != -1){
				texts.set(POS_TEXT_1, content_aux.substring(0, content_aux.indexOf("[")));
			}else{
				texts.set(POS_TEXT_1, content_aux);
			}
		}
						...
						if(n_zone == ZONA_2){
							if(texts[POS_TEXT_1] != ""){
								if(num_lineas && num_lineas != null){
									zone.fillText(texts, num_lineas, n_zone-1);
								}else{
	...
	public String comprobarHex(String value){
		String respuesta = ""
	
		if(value.indexOf(":")==2){
		    respuesta = value.replace(":","");
		    byte[] bytes = javax.xml.bind.DatatypeConverter.parseHexBinary(respuesta);
