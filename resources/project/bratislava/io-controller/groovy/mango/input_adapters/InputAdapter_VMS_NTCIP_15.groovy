...
class InputAdapter_VMS {
			...
			for(int i = 0; i < messages.size(); i++){
				if (messages[i].charAt(0) == "g"){
					if(messages[i].length() < 4){
						graphics.set(i, messages[i].substring(messages[i].indexOf("g") + 1))
					}else{
	...
	class Zone{
		...
		public void fillText(List<String> messages, Integer numLineas, Integer n_zone) {
			if (messages.get(n_zone) || messages.get(n_zone+1)){
				List<String> values = new ArrayList();
				List<String> alternances;
				String aux_value;
				values.add(messages.get(n_zone))
				values.add(messages.get(n_zone+1))
				Text textLine;
