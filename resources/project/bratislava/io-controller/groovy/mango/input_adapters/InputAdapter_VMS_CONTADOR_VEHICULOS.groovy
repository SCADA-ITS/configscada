	...
	class Zone{
		...
		public void fillText(List<String> messages, Integer numLineas, Integer n_zone) {
			...
			if (messages.get(n_zone)){
				...
				Text textLine;
				textLine = new Text(1, values.get(0), null, "#FFFFFF");
				texts.add(textLine);
		...
		public void fillGraphic(List<String> message, Element element, int numZone) {
			Integer values = 0;
					...
					if (numZone == 1){
						values = obtenerGraphic(element,Integer.parseInt(message.get(0)),numZone - 1);
					}else{
						values = obtenerGraphic(element,Integer.parseInt(message.get(1)),numZone - 1);
					}

					Graphic graphic = new Graphic(1, values, 0);
