...
/**
 * 
 * SignallingCommand_53_2: SEM_2_LUCES SWITCH OFF signalling command 
 * 
 */
class SignallingCommand_53_2 {

	GroovyShell shell;
	def signallingCommandUtils;
	static final String LIGHT_CMD = "light_status";
	static final Long CMD_OFF_RR = 0L;
	static final Long CMD_OFF_AA = 1L;
	
	static final Long SEM_SUBTYPE_AMBER = 1L;
	static final Long SEM_SUBTYPE_RED = 2L;
	
	org.apache.logging.log4j.Logger log;
	
	
	SignallingCommand_53_2(org.apache.logging.log4j.Logger log) {
	...
	boolean process(String dataSourceXid, SignallingCommand signallingCommand, MangoDriver driver) {
		...
		try {
			...
			if (element.elementSubtypeId == SEM_SUBTYPE_AMBER ) {
				xidPointValueTimeModel = signallingCommandUtils.getXidPointValueTimeModel(signallingCommand, 
					dataSourceXid + "_" + LIGHT_CMD, CMD_OFF_AA);
			}else if (element.elementSubtypeId == SEM_SUBTYPE_RED ){
				xidPointValueTimeModel = signallingCommandUtils.getXidPointValueTimeModel(signallingCommand, 
					dataSourceXid + "_" + LIGHT_CMD, CMD_OFF_RR);
			}
