...
/**
 *
 * InputAdapter_LUM.groovy
 *
 */
class InputAdapter_LUM {
    ...
    final Long PARAM_MEASURE_LUMINOSITY = 2

    InputAdapter_LUM(org.apache.logging.log4j.Logger log) {

		this.log = log;
	}
	
	boolean luminosity_real(Element element, String value, List<ElementValue> elementValues) {

        double valor = calcular_entrada_ma(Float.parseFloat(value))
        double result = factor_conversion_luminosity_real(valor)

        ElementValue elementValue = new ElementValue();
        elementValue.setElementTypeId(element.getElementTypeId());
        elementValue.setElementTypeParamId(PARAM_MEASURE_LUMINOSITY);
    ...
    double factor_conversion_luminosity_real(double value){
        double result = (5000/16) * (value - 4)
