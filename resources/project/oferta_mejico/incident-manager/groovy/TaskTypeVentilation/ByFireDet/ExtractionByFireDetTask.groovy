...
class ExtractionByFireDetTask {

	static final Long ELEMENT_TYPE_FIRE_DET = 15L;

	org.apache.logging.log4j.Logger log;

	ExtractionByFireDetTask(org.apache.logging.log4j.Logger log) {
	...
	List<Element> getElements(ImsIncidentTypeTask incidentTypeTask, ImsIncidentTypeStretch incidentTypeStretch) {
		...
		if (incidentTypeTask != null && incidentTypeStretch != null ) {
			...
			if (!CollectionUtils.isEmpty(fireDetIds)) {
				...
				log.info("ExtractionByFireDetTask - fireDetId: " + fireDetId);
				...
				if (!CollectionUtils.isEmpty(elementHierarchies)) {
					...
					log.info("ExtractionByFireDetTask - tubeId: " + tubeId);
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						log.info("ExtractionByFireDetTask - ventilationId: " + ventilationId);
			...
			if (!CollectionUtils.isEmpty(fireDetIds)) {
				...
				if (!CollectionUtils.isEmpty(elementHierarchies)) {
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						elementState.setStateId(ElementTypeState.STATE_EXTRACTION);
