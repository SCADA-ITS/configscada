...
class StartVentilationByFireDetTask {

	static final Long ELEMENT_TYPE_FIRE_DET = 15L;

	org.apache.logging.log4j.Logger log;

	StartVentilationByFireDetTask(org.apache.logging.log4j.Logger log) {
	...
	List<Element> getElements(ImsIncidentTypeTask incidentTypeTask, ImsIncidentTypeStretch incidentTypeStretch) {
		...
		if (incidentTypeTask != null && incidentTypeStretch != null ) {
			...
			if (!CollectionUtils.isEmpty(fireDetIds)) {
				...
				log.info("StartVentilationByFireDetTask - fireDetId: " + fireDetId);
				...
				if (!CollectionUtils.isEmpty(elementHierarchies)) {
					...
					log.info("StartVentilationByFireDetTask - tubeId: " + tubeId);
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						log.info("StartVentilationByFireDetTask - ventilationId: " + ventilationId);
			...
			if (!CollectionUtils.isEmpty(fireDetIds)) {
				...
				if (!CollectionUtils.isEmpty(elementHierarchies)) {
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						// STATE_UP
						setElementStatesToVentilationCommand = new SetElementStatesToVentilationCommand();
