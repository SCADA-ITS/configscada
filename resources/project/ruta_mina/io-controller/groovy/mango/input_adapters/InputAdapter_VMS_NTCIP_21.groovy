...
class InputAdapter_VMS {
	...
	final String TEXT_COLOR = "#E2943A";
	...
	public String construirVMS(String content, Element element) {
		List<String> messages = Arrays.asList(content.replace("[jp3][jl3][fo3]","").replace("[np]", "&").split("&"));
		...
		for (String message : messages) {
			...
			if(message.indexOf("[") == 0) {
				...
				texts.add(message.substring(message.indexOf("]") + 1).replace("[nl2]","#"));
			}else {
				texts.add(message.replace("[nl2]","#"));
			}
		}
		...
		if(num_zones && num_zones != null){
			for(int n_zone = 1; n_zone <= num_zones; n_zone++){
			    ...
			    if(alternance != null && flashing_on != null &&  flashing_off != null){	    
					zone = new Zone(n_zone, alternance, flashing_on, flashing_off, "center");
				}else{
