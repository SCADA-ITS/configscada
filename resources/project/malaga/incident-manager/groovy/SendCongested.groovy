...
class SendCongested {
	...
	static final String CONGESTED_VALUE = "0";
	...
	List<Command> process(ImsIncidentReport incidentReport, ImsIncidentTypeTask incidentTypeTask, List<ImsIncidentTypeTaskValue> values) {
		...
		Element tube = EntitiesManager.getInstance().getElement(incidentReportAlarms.get(0).getElementTypeId(), incidentReportAlarms.get(0).getElementId());
		...
		signallingCommand.setElementId(tube.getId());
		...
		signallingParam.setValue(CONGESTED_VALUE);
