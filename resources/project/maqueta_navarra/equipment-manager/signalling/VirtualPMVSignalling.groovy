 ...
 class VirtualPMVSignalling {
	...
	org.apache.logging.log4j.Logger log;
	 ...
	 List<Command> process(PmCommandElement command) {
		...
		for (PmCommandElementValue pmCommandElementValue : command.getPmCommandElementValues()) {

				if (pmCommandElementValue.getCommandElementTypeParamId() != null
						&& pmCommandElementValue.getCommandElementTypeId() != null
						&& pmCommandElementValue.getElementTypeId() != null) {
					
					
					listElements.add(elementSetValue(PARAM_MEASURE_DATA_JSON, TYPE_PARAM_MEASURE, command.getElementId(), pmCommandElementValue.getValue()));	
				}
		}
