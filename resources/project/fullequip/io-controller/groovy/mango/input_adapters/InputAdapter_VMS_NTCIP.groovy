...
class InputAdapter_VMS {
	...
	public String construirVMS(String content, Element element, boolean intermitence) {
		...
		for (String message : messages) {
			...
			if (message.indexOf("g") == -1){
				...
				while(aux_grap != null){
					aux_grap = message.substring(message.indexOf("[g"), message.indexOf("]") + 1);
					
					graphics.add(aux_grap.substring(aux_grap.indexOf("g") + 1, aux_grap.indexOf(",")));
					message = message - aux_grap;
					
					if(message.indexOf("[g") == -1) aux_grap = null;
				}
	...
	class Zone{
		...
		public void fillGraphic(List<String> message, Element element, int numZone, int alternance) {
			...
			ObjectMapper Obj = new ObjectMapper();
			String json = Obj.writeValueAsString(graphics);
		}
