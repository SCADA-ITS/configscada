...
class InputAdapter_VMS {
	...
	public String construirVMS(String content, Element element) {
		List<String> messages = Arrays.asList(content.split(".bmp#"));
		...
		for (String message : messages) {
			...
			graphics.add(elements[1].replace(".bmp",""));
		}
	...
	class Zone{
		...
		public void fillGraphic(List<String> message, Element element, int numZone) {
			...
			if (message.get(0).equals('0')){
				log.error("No existe el pictograma para el mensaje fijo");
				values = 0;
			}else{
			...
			if (message.size() > 1) {
				if (message.get(1).equals('0')){ 
					log.error("No existe el pictograma para el mensaje alternante");
					alternances = 0;
				}else{
