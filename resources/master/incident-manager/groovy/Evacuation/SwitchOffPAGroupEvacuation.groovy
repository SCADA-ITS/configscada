...
class SwitchOffPAGroupEvacuation {
	...
	SwitchOffPAGroupEvacuation(org.apache.logging.log4j.Logger log) {
		...
		if (incidentTypeTask != null && incidentTypeStretch != null) {
			...
			if (!CollectionUtils.isEmpty(tubesId)) {
				...
				for (Long tubeId : tubesId) {
					...
					log.debug("SwitchOffPAGroupEvacuation(getElements) tubo afectado 2000:" + tubeId);
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						log.debug("SwitchOffPAGroupEvacuation(getElements) tunnel afectado 2002:" + illuminationId);
						...
						if (!CollectionUtils.isEmpty(elementGroups)) {
							...
							for (ElementHierarchy elementGroup : elementGroups) {		
								log.debug("SwitchOffPAGroupEvacuation(getElements) grupos del tunel 96:" + elementGroup.getChildElementId()); 
								elements = new ArrayList<>();
		...
		if (incidentReport.getAffectionStretchId() != null) {
			...
			if (!CollectionUtils.isEmpty(tubesId)) {
				...
				for (Long tubeId : tubesId) {
					...
					log.debug("SwitchOffPAGroupEvacuation(process) tubo afectado 2000:" + tubeId);
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						for (ElementHierarchy elementHierarchie : elementHierarchies) {
							...
							log.debug("SwitchOffPAGroupEvacuation(process) tunnel afectado 2002:" + illuminationId);
							...
							if (!CollectionUtils.isEmpty(elementGroups)) {
								...
								for (ElementHierarchy elementGroup : elementGroups) {
									...
									log.debug("SwitchOffPAGroupEvacuation(process) grupo donde difundiré el mensaje 96:" + elementGroup.getChildElementId());
									if (elements == null) {
