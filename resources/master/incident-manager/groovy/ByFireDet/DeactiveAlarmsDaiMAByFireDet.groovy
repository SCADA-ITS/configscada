...
class DeactiveAlarmsDaiMAByFireDet {
	...
	DeactiveAlarmsDaiMAByFireDet(org.apache.logging.log4j.Logger log) {
	...
	List<Element> getElements(ImsIncidentTypeTask incidentTypeTask, ImsIncidentTypeStretch incidentTypeStretch) {
		...
		if (incidentTypeTask != null && incidentTypeStretch != null ) {
			...
			if (!CollectionUtils.isEmpty(fireDetIds)) {
				...
				log.info("DeactiveAlarmsDaiCAByFireDet - fireDetId: " + fireDetId);
				...
				if (!CollectionUtils.isEmpty(elementHierarchies)) {
					...
					log.info("DeactiveAlarmsDaiCAByFireDet - tubeId: " + tubeId);
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						log.info("DeactiveAlarmsDaiCAByFireDet - illuminationId: " + illuminationId);
						...
						if (!CollectionUtils.isEmpty(elementHierarchies)) {
							...
							for (ElementHierarchy elementHierarchie : elementHierarchies) {
								...
								if(elementHierarchie.getChildElementId() % 2 == 0){
