...
class SendMessagePAZoneByFireDetTask {
	...
	SendMessagePAZoneByFireDetTask(org.apache.logging.log4j.Logger log) {
	...
	List<Element> getElements(ImsIncidentTypeTask incidentTypeTask, ImsIncidentTypeStretch incidentTypeStretch) {
		...
		if (incidentTypeTask != null && incidentTypeStretch != null ) {
			...
			if (!CollectionUtils.isEmpty(fireDetIds)) {
				...
				log.info("SendMessagePAZoneByFireDetTask - fireDetId: " + fireDetId);
				...
				if (!CollectionUtils.isEmpty(elementHierarchies)) {
					...
					log.info("SendMessagePAZoneByFireDetTask - tubeId: " + tubeId);
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						log.info("SendMessagePAZoneByFireDetTask - illuminationId: " + illuminationId);
