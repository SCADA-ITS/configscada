		...
		if (incidentTypeTask != null && incidentTypeStretch != null ) {
			...
			if (!CollectionUtils.isEmpty(fireDetIds)) {
				...
				if (!CollectionUtils.isEmpty(elementHierarchies)) {
					...
					elements.add(new Element(ElementType.ELEMENT_TYPE_TUBE, tubeId));
				}
			}	
		}
					...
					if (!CollectionUtils.isEmpty(elementHierarchies)) {
						...
						if (valueMode != null) {
							...
							try {
								...
								setIlluminationModeCommand.setElementId(tubeId);
						...
						if (valueRegime != null) {
							...
							try {
								...
								setIlluminationRegimeCommand.setElementId(tubeId);
