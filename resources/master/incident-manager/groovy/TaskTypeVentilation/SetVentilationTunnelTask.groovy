...
class SetVentilationTask {
	...
	SetVentilationTask(org.apache.logging.log4j.Logger log) {
	...
	List<Element> getElements(ImsIncidentTypeTask incidentTypeTask, ImsIncidentTypeStretch incidentTypeStretch) {
		...
		if (incidentTypeTask != null && incidentTypeStretch != null ) {
			...
			Set<Long> illuminationsId = EntitiesManager.getInstance().getElementByStretch(ElementType.ELEMENT_TYPE_VENTILATION,
			...
			if (!CollectionUtils.isEmpty(illuminationsId)) {
				...
				for (Long illuminationId : illuminationsId) {
					...
					List<ElementHierarchy> elementHierarchies = EntitiesManager.getInstance().getsByParent(
							ElementType.ELEMENT_TYPE_VENTILATION, illuminationId, ElementType.ELEMENT_TYPE_TUBE);
							...
							for (ElementHierarchy elementHierarchie : elementHierarchies) {
								...
								if (valueMode != null) {
									...
									try {
										...
										setVentilationModeCommand.setElementId(elementHierarchie.getChildElementId());
								...
								if (valueRegime != null) {
									...
									try {
										...
										setVentilationRegimeCommand.setElementId(elementHierarchie.getChildElementId());
