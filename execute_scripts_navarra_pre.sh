...
echo "Subiendo $ARCHIVO_TAR a $SCADA_HOST..."
sftp $REMOTE_USER@$SCADA_HOST <<EOF
put $ARCHIVO_TAR
EOF

# Conectar por SSH, descomprimir y ejecutar scripts SQL
echo "Conectando a $SCADA_HOST para descomprimir y ejecutar scripts..."
ssh $REMOTE_USER@$SCADA_HOST <<EOF
mkdir -p $REMOTE_PATH
...
./make_param_navarra_pre.sh $PROYECTO $REMOTE_HOST 5432
cd ../../../ritsfront/resources/db
./make_param_navarra_pre.sh $REMOTE_HOST 5432 $SCADA_HOST
EOF
